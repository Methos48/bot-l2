...
const DISCORD_ORIGIN_CHANNEL_ID = process.env.DISCORD_ORIGIN_Cconst { default: makeWASocket, useMultiFileAuthState, DisconnectReason } = require('@whiskeysockets/baileys');
const { Client: DiscordClient, GatewayIntentBits } = require('discord.js');
const fetch = require('node-fetch');
const FormData = require('form-data');
const express = require('express');
...
// --- INICIALIZACIÓN DE DISCORD CON CONTROL DE TIMEOUT ---
...
if (!DISCORD_BOT_TOKEN) {
    ...
    (async () => {
        try {
            console.log('> [Discord] Ejecutando login con promesa...');
            
            const loginPromise = discordClient.login(DISCORD_BOT_TOKEN);
            const timeoutPromise = new Promise((_, reject) => 
                setTimeout(() => reject(new Error('Timeout: El login de Discord tardó más de 10 segundos en responder.')), 10000)
            );

            await Promise.race([loginPromise, timeoutPromise]);
            console.log('> [Discord] ¡Promesa de login resuelta correctamente!');
        } catch (err) {
            console.error('> [Discord CRITICAL ERROR]:', err.message || err);
        }
    })();
}
// --------------------------------------------------------
